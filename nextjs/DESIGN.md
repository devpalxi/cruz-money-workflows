...
- **Sticky bottom CTA, patron mobile surface only**: in `app/verify/**` the flow CTA sits in a bar fixed to the bottom of the viewport (`sticky bottom-0 bg-white border-t border-border`, padded with `env(safe-area-inset-bottom, 0px)`), not on the page background below the card. This is the single documented exception to the CTA placement rule above: the rule assumes a tablet or desktop viewport where the button is visible after the content, and on a phone that buries it below the fold on every step. It applies nowhere else, and the Collector flow on iPad keeps the standard placement.
...
- **A step handed to someone else is "waiting", not "skipped"**: when the collector sends a patron a self-service verification link, steps 4 to 6 move to the patron's phone. Those steps stay in the stepper and on the summary with a muted "(with patron)" note, using exactly the same neutral `ink-lo` treatment as "(not required)". The two notes are different facts - one step does not apply to this payout, the other applies but is being completed elsewhere - so they read differently while looking the same. Never remove either from the flow.

---

## 6. Patron Self-Service Surface (`app/verify/[token]`)

The only screen in the system a member of the public uses directly. It is not a role portal: no `AdminShell`, no `AppHeader`, no role badge, no operator stepper. Staff chrome on a patron's phone leaks internal workflow to a customer.

- **Same design system, different shell.** Every token, primitive and casing rule above applies unchanged: brand teal, `Card`, `Button`, `Input`, `Checkbox`, status pills, sentence case, `font-mono` for amounts and IDs. Only the layout is new.
- **Shell**: single column capped at `max-width: 480px` and centred, so a desktop viewer still sees the phone view. Compact top bar carrying venue name and total win, a thin brand-teal progress bar with "Step N of M" as plain text (never a pill, per §2.2), content, then the sticky CTA bar.
- **Capture tiles**: document and selfie capture reuse the Relational Association Panel shape (`border border-dashed border-border-mid rounded-lg bg-white p-5`) wrapping a visually hidden `<input type="file" accept="image/*">` with `capture="environment"` for documents and `capture="user"` for the face. On a real phone this opens the camera directly; on desktop it opens a file picker. Once captured the tile shows the image with a "Retake photo" text link.
- **Copy is written for a stressed member of the public, not an operator.** Plain language, no internal vocabulary: "Take a photo of your face", not "Liveness check"; "These details do not match", not "CoP no match". Acronyms the patron actually needs (ID, BSB) stay; the rest (IDV, CoP, DVS, AML) never appear on this surface.
- **Terminal screens** (done, return to the counter, link expired, link closed, link not valid) share one centred layout: a 56px tinted circle with an icon, a heading, and one short paragraph. Tone follows the semantic state colours - teal for success, amber for "something needs staff", red for an invalid link.
- **The patron never gets a staff override.** Anything that exists at the counter as a supervisor judgement, such as the Confirmation of Payee bypass with written justification, is absent here. A patron who cannot pass a check is handed back to staff, never given a way to wave it through.
